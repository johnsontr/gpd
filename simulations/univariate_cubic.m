...
b1 = 1; b2 = 2; b3 = 3;
y = b0 * ones(N,1) + b1*X + b2*X.^2 + b3*X.^3 + noise; % Marginal effects are b1 = 1, b2 = 2, ..., bD = D
train_X = normalize(X); % Normalize training data
train_y = normalize(y); % Normalize training data

% True DGP: y = -1 + X + 2*X^2 + 3*X^3+ e;
% True ME: dy/dx = 1 + 4*X + 9*X.^2 + e
...
univariate_cubic_iso  = plotme(d, d_Xaxis, hyp_iso, meanfunc, covfunc, X, y, Xs);

% Add the true marginal effect to the plot
hold on;
ylim([-20 80]) % Set more reasonable y-axis limits
plot(Xs(:,d_Xaxis), b1 + (2*b2)*Xs(:,d_Xaxis) + (3*b3)*Xs(:,d_Xaxis).^2, ':', 'Color', [0.8500 0.3250 0.0980],'LineWidth', 2, 'DisplayName', "True marginal effect");
